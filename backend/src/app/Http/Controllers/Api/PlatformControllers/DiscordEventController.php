    ...
    SourceEventControllerInterface
{
    ...
    public function handleSingle(SourceSingleEventRequestInterface $request): JsonResponse
    {
        try{
            ...
            $event = $this->discrodEventService->handleSingle($dto);

            return response()->json([
                'success' => true,
                'message' => 'Discord event received successfully',
                'data' => [
                    'event_id'  => $event->id,
    ...
    public function handleBatch(SourceBatchEventRequestInterface $request): JsonResponse
        {   
            try {
                ...
                $results = [];
                foreach ($dtos as $dto) {
                    $results[] = $this->discrodEventService->handleSingle($dto);
                }
    
                return response()->json([
                    'success' => true,
                    'message' => 'Batch of Discord events processed successfully',
                    'count'   => count($results),
                ], 201);
