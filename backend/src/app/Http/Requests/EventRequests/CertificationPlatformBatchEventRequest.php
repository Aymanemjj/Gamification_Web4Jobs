...
use App\DTOs\SourcesEvents\CertificationPlatformEventDTO;
...
class CertificationPlatformBatchEventRequest extends FormRequest implements
    SourceBatchEventRequestInterface
{
    ...
    public function rules(): array
    {
        ...
        foreach ($this->all() as $index => $item) {
            $rules["$index.source"] = "required|string|in:certification_platform";
            ...
            $rules["$index.certification_id"] = "nullable|string|max:255";
            $rules["$index.block_id"] = "nullable|string|max:255";
    ...
    {
        $data = $this->validated();
        return CertificationPlatformEventDTO::collection($data);
    }
}